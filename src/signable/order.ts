...
export class SignableOrder extends Signable<StarkwareOrder> {
  ...
  static fromOrderWithNonce(
    ...
    const quantumsAmountFee = humanAmountToQuantums(order.humanLimitFee, COLLATERAL_ASSET);
  ...
  protected calculateHash(): BN {
    ...
    if (expirationTimestampBn.bitLength() > ORDER_FIELD_BIT_LENGTHS.expirationTimestamp) {
      throw new Error('SignableOrder: expirationTimestamp exceeds max value');
    }
